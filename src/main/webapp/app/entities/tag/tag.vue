<template>
  <div>
    <h2 id="page-heading" data-cy="TagHeading">
      <span v-text="$t('ecomApp.tag.home.title')" id="tag-heading">Tags</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('ecomApp.tag.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'TagCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-tag">
            ...
            <span v-text="$t('ecomApp.tag.home.createLabel')"> Create a new Tag </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && tags && tags.length === 0">
      <span v-text="$t('ecomApp.tag.home.notFound')">No tags found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('ecomApp.tag.name')">Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="ecomApp.tag.delete.question" data-cy="tagDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-tag-heading" v-text="$t('ecomApp.tag.delete.question', { id: removeId })">
