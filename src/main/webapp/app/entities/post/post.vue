<template>
  <div>
    <h2 id="page-heading" data-cy="PostHeading">
      <span v-text="$t('ecomApp.post.home.title')" id="post-heading">Posts</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('ecomApp.post.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PostCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-post">
            ...
            <span v-text="$t('ecomApp.post.home.createLabel')"> Create a new Post </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && posts && posts.length === 0">
      <span v-text="$t('ecomApp.post.home.notFound')">No posts found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('title')">
              <span v-text="$t('ecomApp.post.title')">Title</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'title'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('content')">
              <span v-text="$t('ecomApp.post.content')">Content</span>
            ...
            <th scope="row" v-on:click="changeOrder('date')">
              <span v-text="$t('ecomApp.post.date')">Date</span>
            ...
            <th scope="row" v-on:click="changeOrder('blog.name')">
              <span v-text="$t('ecomApp.post.blog')">Blog</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="ecomApp.post.delete.question" data-cy="postDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-post-heading" v-text="$t('ecomApp.post.delete.question', { id: removeId })">
