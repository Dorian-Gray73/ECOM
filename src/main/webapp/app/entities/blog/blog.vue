<template>
  <div>
    <h2 id="page-heading" data-cy="BlogHeading">
      <span v-text="$t('ecomApp.blog.home.title')" id="blog-heading">Blogs</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('ecomApp.blog.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'BlogCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-blog">
            ...
            <span v-text="$t('ecomApp.blog.home.createLabel')"> Create a new Blog </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && blogs && blogs.length === 0">
      <span v-text="$t('ecomApp.blog.home.notFound')">No blogs found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="blogs">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('ecomApp.blog.name')">Name</span></th>
            <th scope="row"><span v-text="$t('ecomApp.blog.handle')">Handle</span></th>
            <th scope="row"><span v-text="$t('ecomApp.blog.user')">User</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="ecomApp.blog.delete.question" data-cy="blogDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-blog-heading" v-text="$t('ecomApp.blog.delete.question', { id: removeId })">
